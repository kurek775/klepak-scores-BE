...
def test_register_first_user_becomes_admin(client: TestClient):
    resp = client.post(
        "/auth/register",
        json={"email": "first@test.com", "password": "password123", "full_name": "First User"},
    )
...
def test_register_second_user_becomes_evaluator(client: TestClient):
    client.post("/auth/register", json={"email": "a@test.com", "password": "password123", "full_name": "A"})
    resp = client.post("/auth/register", json={"email": "b@test.com", "password": "password123", "full_name": "B"})
    assert resp.status_code == 201
    data = resp.json()
...
def test_register_duplicate_email_409(client: TestClient):
    client.post("/auth/register", json={"email": "dup@test.com", "password": "password123", "full_name": "Dup"})
    resp = client.post("/auth/register", json={"email": "dup@test.com", "password": "password123", "full_name": "Dup2"})
...
def test_login_success(client: TestClient):
    client.post("/auth/register", json={"email": "user@test.com", "password": "password123", "full_name": "User"})
...
def test_login_wrong_password_401(client: TestClient):
    client.post("/auth/register", json={"email": "user2@test.com", "password": "password123", "full_name": "User"})
